...
describe('簡易網誌系統', function () {
  ...
  describe('POST /post', function () { // 路徑 POST /post
...
describe('GET /', function () { // 路徑 GET /
  it('內文標題應該為《貼文列表》，而且只有 1 則貼文', function (done) {
...
describe('GET /post/2', function () {
  it('應該會看到第 2 則貼文', function (done) {
    request.get('/post/1').expect(200, function (err, res) {
      ...
      expect(res.header['content-type']).to.include('html')
      expect(res.text).to.include('<h1>貼文 2</h1>')
      expect(res.text).to.include('<p>內容 2</p>')
      done()
    })
  })
})
